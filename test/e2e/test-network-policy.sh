...
preflight() {
  ...
  if ! command -v expect >/dev/null 2>&1; then
    ...
    sudo apt-get update -qq && sudo apt-get install -y -qq expect >/dev/null 2>&1 || true
  fi
...
\"" 2>&1) || true
  ...
  if echo "$response" | grep -qiE "STATUS_403|ERROR|TIMEOUT"; then
...
test_net_02_whitelist_access() {
  ...
  local response
  response=$(sandbox_exec "pip download --no-deps --dest /tmp/pip-test requests 2>&1 && echo PIP_OK || echo PIP_FAIL" 2>&1) || true
...
\"" 2>&1) || true
  ...
  if echo "$after" | grep -qE "STATUS_[234]"; then
...
\"" 2>&1) || true
  ...
  if echo "$after" | grep -qE "STATUS_[23]"; then
...
\"" 2>&1) || true
  ...
  if echo "$direct_response" | grep -qiE "STATUS_403|ERROR|TIMEOUT"; then
...
test_net_05_hot_reload() {
  ...
  log "  Capturing sandbox PID before policy change..."
  local pid_before
  pid_before=$(sandbox_exec "cat /proc/1/stat 2>/dev/null | awk '{print \$1}'" 2>&1) || true
  log "  PID before: $pid_before"
  ...
  log "  Capturing sandbox PID after policy change..."
  local pid_after
  pid_after=$(sandbox_exec "cat /proc/1/stat 2>/dev/null | awk '{print \$1}'" 2>&1) || true
  log "  PID after: $pid_after"

  if [[ -n "$pid_before" && -n "$pid_after" && "$pid_before" == "$pid_after" ]]; then
    pass "TC-NET-05: Sandbox PID unchanged after policy-add (no restart)"
  elif [[ -z "$pid_before" || -z "$pid_after" ]]; then
    skip "TC-NET-05" "Could not capture sandbox PID"
  else
    fail "TC-NET-05: Hot-reload" "Sandbox PID changed ($pid_before → $pid_after) — sandbox was restarted"
  fi
}
...
test_net_06_permissive_mode() {
  ...
  local permissive_path="nemoclaw-blueprint/policies/openclaw-sandbox-permissive.yaml"
  openshell policy set --policy "$permissive_path" --wait "$SANDBOX_NAME" 2>&1 | tee -a "$LOG_FILE" || true
  sleep 5
...
test_net_09_ssrf_validation() {
  ...
  local result
  result=$(node -e "
const { isPrivateIp } = require('./nemoclaw/dist/blueprint/ssrf');
