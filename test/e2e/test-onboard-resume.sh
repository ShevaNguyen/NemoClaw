...
if [ -f "$SESSION_FILE" ]; then
  info "Session file appeared — killing onboard (PID $onboard_pid) to simulate interruption..."
  ...
  fail "Session file missing after 5 minutes"
  echo "$first_output"
  rm -f "$FIRST_LOG"
  exit 1
fi
...
if echo "$first_output" | grep -q "Sandbox '${SANDBOX_NAME}' created\|Creating sandbox"; then
...
pass "Onboard session file created (confirmed in kill loop above)"

node -e '
const fs = require("fs");
const file = process.argv[1];
const data = JSON.parse(fs.readFileSync(file, "utf8"));
// After kill-mid-flight, session may be in-progress or failed at any step
if (!data.steps || typeof data.steps !== "object") process.exit(1);
' "$SESSION_FILE"
case $? in
  0) pass "Session file contains valid onboard state" ;;
  *) fail "Session file is missing or malformed" ;;
esac
